...
import com.pcy.movierecommendation.spark.util.Helpers.GenericObservable
 ...
 * 实时推荐服务
 *
 * nohup  /opt/module/spark-3.0.0-bin-hadoop3.2/bin/spark-submit --class com.pcy.movierecommendation.spark.service.StreamingRecommender --driver-memory 2G --executor-memory 2G /opt/module/movie_recommender/StreamingRecommender-1.0-SNAPSHOT-jar-with-dependencies.jar > /opt/module/movie_recommender/streaming_recommend_run.log & *
 *
 * ps -ef | grep SparkSubmit | grep -v grep |awk '{print $2}' | xargs -n1 kill
 *
 * @author PengChenyu
...
object StreamingRecommender {
  ...
  def main(args: Array[String]): Unit = {
    // 环境配置
    val sparkConf: SparkConf = new SparkConf().setMaster("local[*]").setAppName("StreamingRecommender")
